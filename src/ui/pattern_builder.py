...
def build_pattern_from_example(example: str) -> tuple[str, str]:
    ...
    elif re.search(r'^[IVXLCDM]+\.\s+\*\s*$', example) or re.search(r'^\d+\.\s+\*\s*$', example):
    ...
    # Try Roman numerals (must be standalone word, typically 1-5 chars)
        ...
        if any(num in example.upper() for num in spelled_numbers):
            return "", f"Pattern '{example}' contains spelled-out numbers. Use wildcard: replace the number with * (e.g., 'THE * BOOK' or 'CHAPTER *')"

        return "", f"Example '{example}' must contain a number (2, 3, ...) or Roman numeral (I, II, III, ...)"
    ...
    elif re.search(r'^\s*[IVXLCDM]+\.\s*$', example) or re.search(r'^\s*\d+\.\s*$', example):
        # Case 2: Just a bare numeral with period (like "II." or "2.")
        pattern = f"^{pattern}\\s+.+$"
        desc += " (matches lines starting with this pattern)"
    elif re.search(r'\w+\s+[IVXLCDM]+\.\s*$', example) or re.search(r'\w+\s+\d+\.\s*$', example):
...
def validate_pattern_on_file(pattern: str, file_path: str, min_chapters: int = 2) -> tuple[bool, str, list]:
    """
    ...
    try:
        ...
        if len(matches) < min_chapters:
            return False, f"Only found {len(matches)} match(es). Need at least {min_chapters}.", matches
        ...
        if span < total_lines * 0.1:
            return False, f"Matches are too close together ({span} lines). May not be actual chapters.", matches
